...
- **Управление прокси клиентов** — создание и удаление прокси прямо из карточки клиента
- **Настройки .env** — редактирование переменных окружения прямо из панели
...
- **Баланс** — пополнение через ЮКассу, оплата заказов с баланса, авто-продление с автосписанием
- Оплата через YooKassa или с баланса (два способа на выбор)
- Личный кабинет: мои прокси, статистика, QR-код для подключения, карточка баланса
...
- Удаление ненужных прокси с автоотключением на сервере
- Повтор платежа для отменённых оплат
- История платежей
...
| POST | `/balance/topup` | Пополнение баланса через ЮКассу |
| POST | `/orders/:id/pay-balance` | Оплата заказа с баланса |
| DELETE | `/orders/:id` | Удаление прокси |
| POST | `/payments/:id/retry` | Повтор платежа |
...
- **Оплата с баланса** — два способа оплаты в модалке подтверждения (баланс + ЮКасса)
- **Авто-продление из баланса** — автоматическое списание и продление прокси
- **Админ: настройки .env** — редактирование переменных окружения из панели
- **Админ: управление прокси клиента** — создание/удаление прокси из карточки клиента
- **Git commit hash** — отображение коммита в модалке версий
- Новые API-эндпоинты: `GET /balance`, `POST /balance/topup`, `POST /orders/:id/pay-balance`

### v2.3.0 (2026-03-19)
- Кнопка «Повторить» для отменённых платежей в истории и на странице результата
- Splash-экран с анимацией при загрузке приложения
- Удаление прокси клиентами из списка «Мои прокси» с автоотключением
- Новые API-эндпоинты: `POST /payments/:id/retry`, `DELETE /orders/:id`
